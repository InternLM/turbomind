...
from .packing_utils import dequantize_gemm
...
try:
    ...
    AWQ_INSTALLED = False
    warnings.warn(
        f'AutoAWQ could not load GEMM kernels extension. Details: {ex}')
...
class WQLinearMMFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if not AWQ_INSTALLED:
            raise ValueError(
                'auto-awq kernels is needed to be installed to '
                'use `.backward()`. Make sure to install the auto-awq kernels'
                ' by following the installation guides in '
                'https://github.com/casper-hansen/AutoAWQ_kernels')

        # Cast to correct dtype for mixed precision training
        weights = awq_ext.dequantize_weights_cuda(qweight, scales, qzeros, 1,
                                                  0, 0,
                                                  False).to(grad_output.dtype)

        if ctx.needs_input_grad[0]:
            # 3D matmul using torch.bmm:
            # https://pytorch.org/docs/stable/generated/torch.bmm.html#torch.bmm # noqa
...
class WQLinear_GEMM(nn.Module):
    ...
    def from_linear(cls,
        ...
        # Avoid: The operator 'aten::__lshift__.Scalar' is not currently
