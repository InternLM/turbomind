...
qbits_available = importlib.util.find_spec(
    'intel_extension_for_transformers') is not None
...
def simple_dispatch_model(model, device_map):
    ...
    # set first cpu offload module's prev_module_hook
    # to the last cpu offload module's hook
    if len(cpu_offload_group) > 1:
        get_module_by_name_suffix(
            model,
            cpu_offload_group[0][0])._hf_hook.prev_module_hook = prev_hook

    for n, d in device_map.items():
        m = get_module_by_name_suffix(model, n)
        if d != 'cpu':
            d = torch.device(d)
            hook = AlignDevicesHook(d,
                                    io_same_device=True,
