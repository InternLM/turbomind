...
#define TM_CHECK(val) myAssert(val, __FILE__, __LINE__)
#define TM_CHECK_WITH_INFO(val, info)                                                                                  \
    do {                                                                                                               \
...
#define TM_THROW(info) throwRuntimeError(__FILE__, __LINE__, info)
    ...
    const T1* pred, const T2* ref, const int size, const int print_size = 0, const std::string filename = "")
{
    ...
    if (print_size > 0) {
        printf("  id |   pred  |   ref   |abs diff | rel diff (%) |");
    }
    ...
    printf("mean_abs_diff: % 6.4f, mean_rel_diff: % 6.4f (%%)", mean_abs_diff, mean_rel_diff);
